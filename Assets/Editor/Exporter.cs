...
using System.Xml;
 ...
 public class Exporter
 {
    ...
    public static void SaveVersionFile(GamePlatformType pt, ExportType export)
    {
        ...
        for(int i = 0; i < clientList.ChildNodes.Count; ++i)
        {
            ...
            else
            {
                ...
                string winDir = ExporterTools.GetRootDir(Exporter.VERSION_DIR) +str+"/正浩捕鱼3D_Data/";
    ...
    public static void PackDir(ResType res, BuildTarget bo, bool bcompress, bool bHD)
    {
        ...
        if (path.Length != 0)
        {
            ...
            if (bo == BuildTarget.Android)
                platform = GamePlatformType.Android.ToString();
            else if (bo == BuildTarget.iOS)
                platform = GamePlatformType.IOS.ToString();
            else if (bo == BuildTarget.StandaloneWindows)
                platform = GamePlatformType.Windows.ToString();
            else
                platform = GamePlatformType.OSX.ToString();
    ...
    public static void PackAllCompressResAll1()
    {
     ...
     public static void PackAllCompressdddD()
     {
         ...
         SaveVersionFile(GamePlatformType.Android, ExportType.Publish);
         SaveVersionFile(GamePlatformType.IOS, ExportType.Publish);
         SaveVersionFile(GamePlatformType.Windows, ExportType.Publish);
         Debug.Log("生成所有XML完成.");
     }
     ...
     public static void PackAllCompressdddD222()
     {
         CreatePublishDir();
         CopyClientToPublishDir();
         SaveVersionFile(GamePlatformType.Android, ExportType.Test);
         SaveVersionFile(GamePlatformType.IOS, ExportType.Test);
         SaveVersionFile(GamePlatformType.Windows, ExportType.Test);
         Debug.Log("生成所有XML完成.");
     }
     ...
     public static void RenameHDToSD()
     {
         ...
         for (int n = 0; n < (int)GamePlatformType.MAX; ++n)
         {
             GamePlatformType pt = (GamePlatformType)n;
