    ...
    void OnBtnOnclickMsg(GameObject go)
    {
       ...
       // return;
        ...
        else if (go == m_BtnObject[2])//we chat
        {
            ...
            if (!MyShareSDK.MyshareSDK.shareSdk.IsAuthorized(PlatformType.WeChat))
            {
                MyShareSDK.MyshareSDK.shareSdk.Authorize(PlatformType.WeChat);
            }
            else
            {
                Hashtable authinfo = MyShareSDK.MyshareSDK.shareSdk.GetAuthInfo(PlatformType.WeChat);
      //          LogMgr.Log("OnClickWX-GetAuthInfo" + MiniJSON.jsonEncode(authinfo));

                //        username = (string)authinfo["openID"];
                ...
                password = (string)authinfo["token"];
#elif UNITY_IPHONE
            username = (string)authinfo["uid"];
            password = (string)authinfo["token"];
#endif
        //        LogMgr.Log("OnClickWX-send username and password: " + username + "  " + password);
